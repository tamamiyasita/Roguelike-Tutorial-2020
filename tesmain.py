...
class MG(arcade.Window):
    def __init__(self, width, height):
        ...
        self.pc = arcade.SpriteSolidColor(
            width=32, height=32, color=arcade.color.BATTLESHIP_GREY)
        self.pc.name = "entity"
        self.pc.center_x = 50
        self.pc.center_y = 50
        self.enm = arcade.SpriteSolidColor(
            width=32, height=32, color=arcade.color.RACKLEY)
        self.enm.name = "actor"
        ...
        self.fd = arcade.load_textures(
            r"image/Effect1.png", ([10.0, 20.0, 39.0, 49.0], [16*8, 16*2, 16.0, 16.0]))

    def on_draw(self):
        arcade.start_render()
        arcade.draw_xywh_rectangle_filled(
            380, 120, 220, 300, color=arcade.color.AIR_FORCE_BLUE)
        arcade.draw_xywh_rectangle_filled(
            20, 0, 500, 100, color=arcade.color.BALL_BLUE)
        self.list.draw()
        text = f" HP:{self.hp} self.hp/ self.max_hp"
        arcade.draw_text(text, 20, 0, arcade.color.WHITE)
        # lightning_efc[83].draw_scaled(80, 80)

        size = 65
        margin = 30
        draw_status_bar(size / 2 + margin, 24, size, 10, self.hp, 50)
        self.fd[1].draw_scaled(100, 100)
        ...
        for message in self.messages:
            arcade.draw_text(
                message, 200, y, color=arcade.color.WHITE, anchor_x="right")
        ...
        if self.mouse_over_text:
            ...
            arcade.draw_rectangle_filled(
                x, y+8, 160, 16, arcade.color.BLACK_BEAN)
            arcade.draw_text(self.mouse_over_text, x, y,
