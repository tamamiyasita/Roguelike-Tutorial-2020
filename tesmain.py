...
def make_puff(prev_emitter):
    ...
    return arcade.Emitter(
        ...
        particle_factory=lambda emitter: arcade.FadeParticle(
            ...
            change_xy=(_Vec2(arcade.rand_in_circle((0.0, 0.0), 0.4)
                             ) + _Vec2(0.3, 0.0)).as_tuple(),
            lifetime=4.0
        )
    )
...
class AnimatedAlphaParticle(arcade.LifetimeParticle):
    ...
    def update(self):
        ...
        if self.lifetime_elapsed <= self.in_duration:
            ...
            self.alpha = clamp(arcade.lerp(
                self.start_alpha, self.mid_alpha, u), 0, 255)
        else:
            u = (self.lifetime_elapsed - self.in_duration) / self.out_duration
            self.alpha = clamp(arcade.lerp(
                self.mid_alpha, self.end_alpha, u), 0, 255)
...
class FireworksApp(arcade.Window):
    ...
    def launch_firework(self, delta_time):
        ...
        pyglet.clock.schedule_once(self.launch_firework, random.uniform(
            LAUNCH_INTERVAL_MIN, LAUNCH_INTERVAL_MAX))
